...
var Filter = function Filter(fn, h) {
  this.fn = fn;
  this.h = h;
};

Filter.prototype.go = function(args) {
  var res, h = this.h;
  if ( !this.fn ) {
    h._dispatch( args );
  }
    ...
    if ( !(res instanceof Herald) ) {
      ...
      res.then(function() {
        h._dispatch(arguments);
      });
      res.rescue(function() {
        h._dismiss(arguments);
      });
    }
  }
};
...
Herald = function Herald(filter) {
  ...
  this._thens = [];
  this._rescues = [];
  this._filter = new Filter( filter, this );
};
